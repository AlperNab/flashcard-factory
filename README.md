...
**Domain:** `Education / Study Tools` · **Suite:** `Medical & Research Suite` · **Accent:** `#5eead4`
...
**Flashcard Factory** is a standalone, browser-based AI workflow app for **Education / Study Tools**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
|---|---:|:---:|---|---|
| `document`<br>Document | text | Yes | — | Affects input: Document. |
...
| `subject`<br>subject | text | No | — | Affects customization: subject. |
| `learner_level`<br>learner level | select | No | — | Affects customization: learner level. |
| `card_count`<br>card count | text | No | — | Affects customization: card count. |
| `exam_type`<br>exam type | text | No | — | Affects customization: exam type. |
| `language`<br>language | select | No | English | Affects customization: language. |
| `difficulty`<br>difficulty | text | No | — | Affects customization: difficulty. |
| `cloze_basic_mix`<br>cloze/basic mix | text | No | — | Affects customization: cloze/basic mix. |
| `tagging_scheme`<br>tagging scheme | text | No | — | Affects customization: tagging scheme. |
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9123`. |
...
```bash
curl -X POST http://127.0.0.1:9123/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9123 uvicorn app.main:app --host 0.0.0.0 --port 9123
```
...
- `plugins/flashcard-factory.json` for plugin metadata.
